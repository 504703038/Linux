...
from pub.student_course
group by cid)
elect cid,name,max_score,max_score2,max_score_count
from 

(select x.cid,max(score) as max_score2
from pub.student_course p,x
where p.cid=x.cid and score!=x.max_score
group by x.cid)
natural join
(select x.cid,x.max_score,count(score) as max_score_count
from x,pub.student_course y
where x.cid=y.cid and x.max_score=y.score
group by x.cid,x.max_score)
natural join
pub.course
```
